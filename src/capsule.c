...
/*   Updated: 2021/04/18 19:01:29 by abaudot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float setXyabc(const t_cps *cp, const t_ray *r, float *comp, float *abch)
{
	t_vec3f oa;

	sub_(r->o, cp->pa, oa); // oa
	comp[0] = dotp(cp->ba, r->d); //bard
	comp[1] = dotp(cp->ba, oa); //baoa
	comp[2] = dotp(r->d, oa); //rdoa
	comp[3] = dotp(oa, oa); //oaoa
